...
export function UpcomingMaintenance() {
  ...
  const loadMaintenance = () => {
    ...
    console.log('[v0] Loading upcoming maintenance for user:', user.id)
    ...
    const upcomingBookings = bookings
      ...
      .map((booking: any) => {
        ...
        return {
          ...
          date: daysUntil < 0 ? "Overdue" : new Date(booking.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
          priority,
          icon: booking.service.toLowerCase().includes('oil') ? Droplet : 
                booking.service.toLowerCase().includes('filter') ? Filter : Wrench
        }
      })
      .sort((a: any, b: any) => {
    ...
    console.log('[v0] Upcoming maintenance items:', upcomingBookings.length)
  ...
  useEffect(() => {
    ...
    const handleStorageChange = () => {
      console.log('[v0] Storage event detected, reloading maintenance')
      loadMaintenance()
    }
            ...
            <Card key={index} className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground mb-1">{item.vehicle}</p>
